/**
 * Manufacturing Service — business logic layer
...
export const manufacturingService = {
  ...
  async findAll(tenantId: string, page = 0, limit = 50) {
    ...
    const result = await pool.query(
      "SELECT * FROM work_orders WHERE tenant_id=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3",
  ...
  async softDelete(id: string, tenantId: string) {
    ...
    try {
      await pool.query(
        "UPDATE work_orders SET is_active=false WHERE id=$1 AND tenant_id=$2",
        [id, tenantId]
      )
    } catch {
    ...
    logger.info(`[Manufacturing] Deleted ${id} for tenant ${tenantId}`)
  },
}
