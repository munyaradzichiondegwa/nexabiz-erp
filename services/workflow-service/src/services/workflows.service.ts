/**
 * Workflows Service — business logic layer
...
export const workflowService = {
  ...
  async findAll(tenantId: string, page = 0, limit = 50) {
    ...
    const result = await pool.query(
      "SELECT * FROM workflow_instances WHERE tenant_id=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3",
  ...
  async softDelete(id: string, tenantId: string) {
    ...
    try {
      await pool.query(
        "UPDATE workflow_instances SET is_active=false WHERE id=$1 AND tenant_id=$2",
        [id, tenantId]
      )
    } catch {
      // Table may not have is_active; do hard delete with audit
      await pool.query(
        "DELETE FROM workflow_instances WHERE id=$1 AND tenant_id=$2",
        [id, tenantId]
      )
    }
    logger.info(`[Workflows] Deleted ${id} for tenant ${tenantId}`)
  },
}
