...
export const reportingService = {
  ...
  async findAll(tenantId: string, page = 0, limit = 50) {
    ...
    const result = await pool.query(
      "SELECT * FROM journal_entries WHERE tenant_id=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3",
  ...
  async softDelete(id: string, tenantId: string) {
    ...
    try {
      await pool.query(
        "UPDATE journal_entries SET is_active=false WHERE id=$1 AND tenant_id=$2",
        [id, tenantId]
      )
    } catch {
    ...
    logger.info(`[Reporting] Deleted ${id} for tenant ${tenantId}`)
  },
}
