...
export const budgetingService = {
  ...
  async findAll(tenantId: string, page = 0, limit = 50) {
    ...
    const result = await pool.query(
      "SELECT * FROM budgets WHERE tenant_id=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3",
      [tenantId, limit, offset]
    )
    const count = await pool.query(
      "SELECT COUNT(*) FROM budgets WHERE tenant_id=$1",
      [tenantId]
    )
    return {
      data: result.rows,
  ...
  async softDelete(id: string, tenantId: string) {
    ...
    try {
      await pool.query(
        "UPDATE budgets SET is_active=false WHERE id=$1 AND tenant_id=$2",
        [id, tenantId]
      )
    } catch {
    ...
    logger.info(`[Budgeting] Deleted ${id} for tenant ${tenantId}`)
  },
}
